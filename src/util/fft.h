...
static const int16_t kFftCosQ15[FFT_SIZE / 2] = {32767,  32610,  32138,  31357,  30273,  28898,  27245,  25329,
...
static const int16_t kFftSinQ15[FFT_SIZE / 2] = {0,      -3212,  -6393,  -9512,  -12539, -15447, -18204, -20787,
                                                 -23170, -25329, -27245, -28898, -30273, -31357, -32138, -32610,
  ...
  for (uint16_t len = 2; len <= FFT_SIZE; len <<= 1) {
    ...
    for (uint16_t i = 0; i < FFT_SIZE; i += len) {
      for (uint16_t k = 0; k < halfLen; k++) {
        ...
        const int16_t wr = kFftCosQ15[tw];
        const int16_t wi = kFftSinQ15[tw];
