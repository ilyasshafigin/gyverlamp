...
// максимальная вертикальная скорость перемещения языков пламени за кадр.  имеется в виду 256/256 =   1 пиксель за кадр
#define FLAME_MAX_DY 256
// минимальная вертикальная скорость перемещения языков пламени за кадр.   имеется в виду 128/256 = 0.5 пикселя за кадр
#define FLAME_MIN_DY 128
// максимальная горизонтальная скорость перемещения языков пламени за кадр. имеется в виду 32/256 = 0.125 пикселя за кадр
#define FLAME_MAX_DX 32
#define FLAME_MIN_DX (-FLAME_MAX_DX)
// максимальная начальная яркость языка пламени
#define FLAME_MAX_VALUE 255
// минимальная начальная яркость языка пламени
#define FLAME_MIN_VALUE 176

// базовый шаг симуляции в мс (1000 / 50 FPS). Эффект не зависит от реального FPS.
#define FIRE_STEP_MS 20U
...
void wu_pixel_maxV(int16_t item) {
  ...
  uint8_t xx = (trackingObjectPosX[item] - static_cast<int>(trackingObjectPosX[item])) * 255;
  uint8_t yy = (trackingObjectPosY[item] - static_cast<int>(trackingObjectPosY[item])) * 255;
  uint8_t ix = 255 - xx;
  uint8_t iy = 255 - yy;
...
#define WU_WEIGHT(a, b) (static_cast<uint8_t>(((a) * (b) + (a) + (b)) >> 8))
  ...
  for (uint8_t i = 0; i < 4; i++) {
    uint8_t x1 = static_cast<int8_t>(trackingObjectPosX[item] + (i & 1)) % WIDTH; //делаем бесшовный по ИКСу
    uint8_t y1 = static_cast<int8_t>(trackingObjectPosY[item] + ((i >> 1) & 1));
...
void EffectFire::setup(EffectContext& ctx) {
  ff_y = 0U; // аккумулятор времени симуляции
  int16_t i, j;
...
void EffectFire::render(EffectContext& ctx) {
  ...
  // симуляция с фиксированным шагом — скорость анимации не зависит от FPS
  ff_y += ctx.deltaMs;
  while (ff_y >= FIRE_STEP_MS) {
    ff_y -= FIRE_STEP_MS;
