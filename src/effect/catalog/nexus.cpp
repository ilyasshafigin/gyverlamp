...
static void nexusReset(uint8_t i) {
  ...
  trackingObjectSpeedX[i] = static_cast<float>(random8(5, 11)) / 70 + speedfactor; // делаем частицам немного разное ускорение и сразу пересчитываем под общую скорость
...
void EffectNexus::render(EffectContext& ctx) {
  ...
  for (uint8_t i = 0; i < enlargedObjectNum; i++) {
    ...
    if (ctx.palette) {
      ctx.led.drawPixelSafe(trackingObjectPosX[i], trackingObjectPosY[i], ColorFromPalette(*ctx.palette, trackingObjectHue[i], 255U));
    } else {
