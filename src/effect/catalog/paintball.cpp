...
void EffectPaintball::render(EffectContext& ctx) {
  ...
  ctx.led.getPixelSafe(highByte(i * paintWidth) + BORDERTHICKNESS, highByte(j * paintHeight) + BORDERTHICKNESS) += ColorFromPalette(palette, ms / 29);
  ctx.led.getPixelSafe(highByte(j * paintWidth) + BORDERTHICKNESS, highByte(k * paintHeight) + BORDERTHICKNESS) += ColorFromPalette(palette, ms / 41);
  ctx.led.getPixelSafe(highByte(k * paintWidth) + BORDERTHICKNESS, highByte(m * paintHeight) + BORDERTHICKNESS) += ColorFromPalette(palette, ms / 37);
  ctx.led.getPixelSafe(highByte(m * paintWidth) + BORDERTHICKNESS, highByte(i * paintHeight) + BORDERTHICKNESS) += ColorFromPalette(palette, ms / 53);
}
