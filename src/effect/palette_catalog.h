...
#define PALETTE_REGISTRY_AUTO(X) \
  X(Palettes::Id::Auto,   "Auto",    nullptr)
...
namespace Palettes {
  ...
  constexpr bool isValid(uint8_t raw) { return raw < COUNT; }
  constexpr Id clamp(uint8_t raw) { return isValid(raw) ? static_cast<Id>(raw) : Id::Auto; }
  constexpr Id clamp(Id id) { return clamp(toIndex(id)); }

  inline Id getPaletteIdByIndex(uint8_t index) { return index >= SELECTABLE_COUNT ? Id::Auto : SELECTABLE_ORDER[index]; }
