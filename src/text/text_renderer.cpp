...
void TextRenderer::drawChar(NotificationOverlay& overlay, int16_t x, int16_t y, uint8_t letter, const CRGB& color, bool wrapX) {
...
void TextRenderer::drawString(NotificationOverlay& overlay, int16_t x, int16_t y, const String& text, const CRGB& color) {
  uint16_t index = 0;
  int16_t cursor = x;
  while (text[index] != '\0') {
    const uint8_t letter = readFontByte(text, index);
