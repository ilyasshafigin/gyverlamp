...
- `OtaService` (`src/network/ota_service.cpp`, compiled in only when `USE_OTA` is defined) defers `ArduinoOTA.begin()` out of `init()` into `tick()`, starting it only after `WifiService` reports a STA connection; it begins once and `handle()`s thereafter.
