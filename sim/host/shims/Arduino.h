...
inline uint32_t micros() { return sim_millis * 1000; }
...
inline int analogRead(uint8_t pin) { return sim::audioReadAdc(pin); }
...
inline void randomSeed(uint32_t seed) { rng_state() = seed ? seed : 123456789; }
...
inline uint8_t random8() { return static_cast<uint8_t>(random_uint32()); }
inline uint8_t random8(uint8_t lim) { return lim ? static_cast<uint8_t>(random_uint32() % lim) : 0; }
inline uint8_t random8(uint8_t min, uint8_t lim) { return (lim > min) ? static_cast<uint8_t>(min + (random_uint32() % (lim - min))) : min; }
inline uint16_t random16() { return static_cast<uint16_t>(random_uint32()); }
inline uint16_t random16(uint16_t lim) { return lim ? static_cast<uint16_t>(random_uint32() % lim) : 0; }
...
public:
    template<typename T>
    FakeSerial& print(T) { return *this; }
    template<typename T>
    FakeSerial& println(T) { return *this; }
