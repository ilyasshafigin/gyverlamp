...
struct CRGB {
    ...
    CRGB operator+(const CRGB& o) const { CRGB t = *this; return t += o; }
    ...
    CRGB operator-(uint32_t c) const { CRGB t = *this; return t -= c; }
...
struct CHSV {
    union { uint8_t h = 0; uint8_t hue; };
    union { uint8_t s = 255; uint8_t sat; };
    union { uint8_t v = 255; uint8_t val; };
    CHSV() = default;
...
inline CRGB hsv2rgb_rainbow(const CHSV& hsv) {
    ...
    switch (hue >> 5) {
        case 0: r = 255;      g = ramp;     b = 0;        break; // red -> orange
        case 1: r = 255;      g = 255;      b = 0;        break; // yellow hold
        case 2: r = rampdown; g = 255;      b = 0;        break; // yellow -> green
        case 3: r = 0;        g = 255;      b = ramp;     break; // green -> aqua
        case 4: r = 0;        g = rampdown; b = 255;      break; // aqua -> blue
        case 5: r = ramp;     g = 0;        b = 255;      break; // blue -> purple
        case 6: r = 255;      g = 0;        b = rampdown; break; // purple -> red
...
inline CRGB hsv2rgb_spectrum(const CHSV& hsv) {
    ...
    uint8_t r, g, b;
    switch (section) {
        case 0:  r = rampinv; g = ramp;    b = 0;        break;
        case 1:  r = 0;       g = rampinv; b = ramp;     break;
        default: r = ramp;    g = 0;       b = rampinv;  break;
    }
    if (hsv.s != 255) {
...
inline void hsv2rgb_spectrum(const CHSV& hsv, CRGB& rgb) { rgb = hsv2rgb_spectrum(hsv); }
...
inline CRGB ColorFromPalette(const CRGBPalette16& pal, uint8_t index, uint8_t brightness = 255, uint8_t blendType = LINEARBLEND) {
...
inline void fill_gradient(CRGB* leds, int index1, const CHSV& c1, int index2, const CHSV& c2, TGradientDirectionCode /*direction*/ = SHORTEST_HUES) {
...
inline uint8_t dim8_raw(uint8_t x) { return x >> 1; }
...
inline void fadeToBlackBy(CRGB* leds, int numLeds, uint8_t step) {
    for (int i = 0; i < numLeds; ++i) leds[i].fadeToBlackBy(step);
}
...
inline uint32_t bpm_to_q88(accum88 beats_per_minute) {
    return (beats_per_minute < 256U)
        ? static_cast<uint32_t>(beats_per_minute) * 256U
...
inline uint16_t beatsin88(accum88 beats_per_minute, uint16_t lowest = 0, uint16_t highest = 65535,
                          uint32_t timebase = 0, uint16_t phase_offset = 0) {
...
inline void blur2d(CRGB* leds, uint8_t width, uint8_t height, fract8 amount, const fl::XYMap& xyMap) {
    ...
    for (uint8_t y = 0; y < height; ++y) {
        for (uint8_t x = 0; x < width; ++x) {
            ...
            CRGB avg(static_cast<uint8_t>(r / neighbors),
                     static_cast<uint8_t>(g / neighbors),
                     static_cast<uint8_t>(b / neighbors));
...
inline CRGBPalette16 make_party_palette() {
    CRGB entries[16] = {
        CRGB(0x5500AB), CRGB(0x84007C), CRGB(0xB5004B), CRGB(0xE4001B),
        CRGB(0xF91700), CRGB(0xF96400), CRGB(0xF9B200), CRGB(0xF9FF00),
        CRGB(0xB2FF00), CRGB(0x64FF00), CRGB(0x17FF00), CRGB(0x00FF1B),
        CRGB(0x00FF68), CRGB(0x00FFB6), CRGB(0x00FFFF), CRGB(0x00ABFF)
    };
    return CRGBPalette16(entries);
}
...
inline CRGBPalette16 make_ocean_palette() {
    CRGB entries[16] = {
        CRGB(0x000050), CRGB(0x000060), CRGB(0x000070), CRGB(0x000080),
        CRGB(0x000090), CRGB(0x0000A0), CRGB(0x0000B0), CRGB(0x0000C0),
        CRGB(0x0000D0), CRGB(0x0000E0), CRGB(0x0000F0), CRGB(0x0000FF),
        CRGB(0x0030FF), CRGB(0x0060FF), CRGB(0x0090FF), CRGB(0x00C0FF)
    };
    return CRGBPalette16(entries);
}
...
inline CRGBPalette16 make_forest_palette() {
    CRGB entries[16] = {
        CRGB(0x001100), CRGB(0x002200), CRGB(0x003300), CRGB(0x004400),
        CRGB(0x005500), CRGB(0x006600), CRGB(0x007700), CRGB(0x008800),
        CRGB(0x009900), CRGB(0x22AA00), CRGB(0x44BB00), CRGB(0x66CC00),
        CRGB(0x88DD00), CRGB(0xAAEE00), CRGB(0xCCFF00), CRGB(0xEEFF44)
    };
    return CRGBPalette16(entries);
}
...
inline CRGBPalette16 make_clouds_palette() {
    CRGB entries[16] = {
        CRGB(0x444444), CRGB(0x555555), CRGB(0x666666), CRGB(0x777777),
        CRGB(0x888888), CRGB(0x999999), CRGB(0xAAAAAA), CRGB(0xBBBBBB),
        CRGB(0xCCCCCC), CRGB(0xDDDDDD), CRGB(0xDDEEEE), CRGB(0xDDFFFF),
        CRGB(0xEEEEFF), CRGB(0xFFFFFF), CRGB(0xFFFFFF), CRGB(0xFFFFFF)
    };
    return CRGBPalette16(entries);
}
...
inline CRGBPalette16 make_lava_palette() {
    CRGB entries[16] = {
        CRGB(0x000000), CRGB(0x1F0000), CRGB(0x3F0000), CRGB(0x5F0000),
        CRGB(0x7F0000), CRGB(0x9F0000), CRGB(0xBF0000), CRGB(0xDF0000),
        CRGB(0xFF0000), CRGB(0xFF1F00), CRGB(0xFF3F00), CRGB(0xFF5F00),
        CRGB(0xFF7F00), CRGB(0xFF9F00), CRGB(0xFFBF00), CRGB(0xFFDF00)
    };
    return CRGBPalette16(entries);
}
...
inline CRGBPalette16 make_heat_palette() {
    CRGB entries[16] = {
        CRGB(0x000000), CRGB(0x330000), CRGB(0x660000), CRGB(0x990000),
        CRGB(0xCC0000), CRGB(0xFF0000), CRGB(0xFF3300), CRGB(0xFF6600),
        CRGB(0xFF9900), CRGB(0xFFCC00), CRGB(0xFFFF00), CRGB(0xFFFF33),
        CRGB(0xFFFF66), CRGB(0xFFFF99), CRGB(0xFFFFCC), CRGB(0xFFFFFF)
    };
    return CRGBPalette16(entries);
}
