...
namespace sim {
  ...
  bool SimRuntime::init(const RuntimeOptions& options) {
    ...
    _notifications = new NotificationController(*_eeprom, *_power, *_runningText, *_stateNotifier, *_time);
  ...
  void SimRuntime::applyCommand(const Command& cmd) {
    switch (cmd.type) {
      ...
      case CommandType::SetBrightness: _effects->setEffectBrightness(static_cast<uint8_t>(cmd.intValue)); break;
      case CommandType::SetSpeed: _effects->setEffectSpeed(static_cast<uint8_t>(cmd.intValue)); break;
      case CommandType::SetScale: _effects->setEffectScale(static_cast<uint8_t>(cmd.intValue)); break;
      case CommandType::ResetEffectSettings: _effects->resetEffectSettingsToDefaults(); break;
      case CommandType::SetFps:
        if (cmd.intValue > 0) {
          _fps.store(static_cast<uint8_t>(cmd.intValue));
        }
        break;
      case CommandType::Pause: _paused.store(cmd.intValue != 0); break;
      case CommandType::Step: _step.store(true); break;
      case CommandType::PowerOn: _power->on(); break;
      case CommandType::PowerOff: _power->off(); break;
      case CommandType::PowerToggle: _power->toggle(); break;
      case CommandType::NextEffect: _effects->setNextEffect(); break;
      case CommandType::PrevEffect: _effects->setPreviousEffect(); break;
      case CommandType::NotifyText:
        _notifications->startUserTextNotification(String(cmd.strValue.c_str()), CRGB::White, SIM_TEXT_NOTIFICATION_MS);
        break;
      case CommandType::NotifyUser:
        if (isUserNotificationType(cmd.intValue)) {
          _notifications->startUserNotification(static_cast<UserNotificationType>(cmd.intValue), 0);
        }
        break;
      case CommandType::StopNotification: _notifications->stopUserNotification(); break;
      case CommandType::ButtonPress:
        if (cmd.intValue >= 1 && cmd.intValue <= 5) {
          buttonPress(static_cast<uint8_t>(cmd.intValue));
        }
        break;
      case CommandType::ButtonRelease: buttonRelease(); break;
      case CommandType::ButtonTap:
        if (cmd.intValue >= 1 && cmd.intValue <= 5) {
          buttonTap(static_cast<uint8_t>(cmd.intValue));
        }
        break;
      case CommandType::ButtonHold: buttonHold(); break;
      case CommandType::ButtonStep: buttonStep(); break;
      ...
      case CommandType::SetAudioAmount:
        ...
        break;
      case CommandType::Quit: _quit.store(true); break;
      default: break;
    }
  }
